    ...
    <a-spin :spinning="loading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        <a-row>
          ...
          <a-col :span="12">
            <a-form-model-item label="所在仓库" prop="WhseId">
              <CommonSelect table="Bas_Whse" v-model="entity.WhseId"></CommonSelect>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="默认货主" prop="StorerId">
              <StorerSelect v-model="entity.StorerId" :type="['Storer']" :whse="entity.WhseId" placeholder="货主"></StorerSelect>
            </a-form-model-item>
          </a-col>
        </a-row>
      </a-form-model>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        WhseId: [{ required: true, message: '必填' }],
  ...
  methods: {
    ...
    init() {
      this.loading = false
      this.visible = true
      this.entity = { Id: '', Code: '', Name: '', UserName: '', Password: '', Sex: '', OrgId: '', WhseId: this.defaultWhseId, StorerId: this.defaultStorerId, Status: 'Enable' }
      this.$nextTick(() => {
