<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="48">
          ...
          <a-col :md="4" :sm="24">
            <a-form-item label="库位">
              <LocSelect v-model="queryParam.PickLocId"></LocSelect>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="24">
    ...
    <s-table ref="table" size="default" rowKey="Id" :columns="columns" :data="loadData" :rowSelection="rowSelection" showPagination="auto">
      <template slot="UomCode" slot-scope="text">
...
const columns = [
  ...
  { title: '拣货库位', dataIndex: 'PickLoc.Code' },
  { title: '最大容量', dataIndex: 'MaxCapacity' },
  { title: '补货单位', dataIndex: 'UomCode', scopedSlots: { customRender: 'UomCode' } },
  { title: '补货触发点', dataIndex: 'MinQty' },
...
export default {
  ...
  data() {
    ...
    return {
      ...
      advanced: false,
      // 查询参数
      queryParam: { WhseId: '', StorerId: '', SkuId: '', PickLocId: '' },
  ...
  methods: {
    ...
    resetSearchForm() {
      this.queryParam = { WhseId: this.defaultWhseId, StorerId: this.defaultStorerId, SkuId: '', PickLocId: '' }
    },
    handleDelete(rows) {
      var thisObj = this
