<template>
  <a-drawer title="仓库权限" placement="right" width="40%" :visible="visible" @close="()=>{this.visible=false;this.$emit('close')}">
...
import MainSvc from '@/api/Bas/Bas_RoleWhseSvc'
import RoleSelect from '../../Sys/Sys_Role/RoleSelect'
...
export default {
  ...
  methods: {
    ...
    handleRoleSelect(roleKeys, roles) {
      MainSvc.Save(this.queryParam.WhseId, roleKeys).then(result => {
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise((resolve, reject) => {
            MainSvc.Delete(thisObj.queryParam.WhseId, ids).then(result => {
              resolve()
              if (result.Success) {
