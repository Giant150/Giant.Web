        ...
        <a-row :gutter="48">
          ...
          <a-col :md="4" :sm="24">
            <a-form-item label="托盘类型">
              <CommonSelect table="Bas_TrayType" v-model="queryParam.TrayTypeId"></CommonSelect>
            </a-form-item>
          </a-col>
          <a-col :md="4" :sm="24">
            <a-form-item label="空托盘">
              <a-select v-model="queryParam.IsEmpty" :allowClear="true">
          ...
          <a-dropdown >
            <a-menu slot="overlay" >
              <a-menu-item key="1">
                <a-button type="primary" icon="arrow-down" @click="handleExport">导出</a-button>
              </a-menu-item>
              <a-menu-item key="2">
                <a-upload name="file" :multiple="true" :action="uploadConfig.action" :headers="uploadConfig.headers" @change="handleChange">

                  <a-button type="primary" icon="arrow-up" >导入</a-button>
                </a-upload>
              </a-menu-item>
              <a-menu-item key="3">
            ...
            <a-button> 导入\出 <a-icon type="download" /> </a-button>
          </a-dropdown>
          <!-- <a-button type="primary">导出</a-button>
          <a-button style="margin-left: 8px" type="primary" @click="hanldleLeading()">导入</a-button> -->
          <!-- <a-button type="primary" icon="export" @click="handleDetailExport" size="small">下载模板</a-button> -->
        </a-row>
      </a-form>
    </div>
...
const columns = [
  ...
  { title: '空托盘', dataIndex: 'IsEmpty', customRender: (value, row, index) => { return value ? '是' : '否' } },
  { title: '所在库位', dataIndex: 'Loc.Code' },
...
export default {
  ...
  data() {
    ...
    return {
      ...
      advanced: false,
      // 查询参数
      queryParam: { WhseId: '', TrayTypeId: '', IsEmpty: '', Keyword: '' },
  ...
  created() {
    ...
    this.uploadConfig.action = `${process.env.VUE_APP_API_BASE_URL}/api/Bas_Tray/Import?whseId=${this.defaultWhseId}`
  },
  methods: {
    moment,
    handleAdd() {
    ...
    resetSearchForm() {
      this.queryParam = { WhseId: this.defaultWhseId, TrayTypeId: '', IsEmpty: '', Keyword: '' }
    },
    handleDelete(rows) {
      var thisObj = this
